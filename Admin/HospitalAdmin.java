...
public class HospitalAdmin extends JFrame {
    ...
    private void showDoctorsManagementDashboard() {
        ...
        for (String dept : DEPARTMENTS) {
            ...
            for (Doctor doc : doctorList) {
                if (doc.getDepartment().equalsIgnoreCase(dept)) {
                    ...
                    System.out.println("[DEBUG table row] " + doc.getName() + " shifts: " + doc.getShiftsFormatted());
        ...
        btnShifts.addActionListener(e -> {
            ...
            int tableIdx = -1, rowIdx = -1;
            for (int i = 0; i < allTables.size(); i++) {
                int sIdx = allTables.get(i).getSelectedRow();
                if (sIdx != -1) { targetDoc = doctorsPerDept.get(i).get(sIdx); tableIdx = i; rowIdx = sIdx; }
            }
            if (targetDoc == null) {
            ...
            if (selected != null) {
                ...
                if (tableIdx != -1 && rowIdx != -1 && tableIdx < allTables.size() && rowIdx < allTables.get(tableIdx).getRowCount()) {
                    allTables.get(tableIdx).getModel().setValueAt(targetDoc.getShiftsFormatted(), rowIdx, 2);
                }
        ...
        btnToggle.addActionListener(e -> {
            for (int i = 0; i < allTables.size(); i++) {
                ...
                if (sIdx != -1) {
                    ...
                    allTables.get(i).getModel().setValueAt(doc.isActive() ? "ACTIVE" : "ON LEAVE", sIdx, 3);
                    if (!doc.isActive()) {
        ...
        btnUpdate.addActionListener(e -> {
            ...
            if (target != null) {
                ...
                // Update the Name and Room columns in the existing table model directly
                for (int i = 0; i < allTables.size(); i++) {
                    for (int r = 0; r < allTables.get(i).getRowCount(); r++) {
                        Doctor d = doctorsPerDept.get(i).get(r);
                        if (d == target) {
                            if (newName != null && !newName.trim().isEmpty())
                                allTables.get(i).getModel().setValueAt("Dr. " + target.getName(), r, 1);
                            if (newRoom != null && !newRoom.trim().isEmpty())
                                allTables.get(i).getModel().setValueAt(target.getRoomNumber(), r, 4);
                            showDoctorsManagementDashboard();
                            return;
                        }
                    }
        ...
        contentScroll.setViewportView(displayPanel);
    ...
    private void showActiveEmergencies() {
        ...
        java.util.List<Object> rowSources = new java.util.ArrayList<>();
        java.util.Set<String> seen = new java.util.LinkedHashSet<>();
        ...
        btnAssignDoc.addActionListener(e -> {
            ...
            if (selectedIdx == -1 || selectedIdx >= rowSources.size()) return;
            Object source = rowSources.get(selectedIdx);
            ...
            if (source instanceof EmergencyCase) {
                ((EmergencyCase) source).setAssignedDoctor(newDoc);
            } else if (source instanceof HospitalSystem.SharedEmergencyCase) {
                HospitalSystem.SharedEmergencyCase sec = (HospitalSystem.SharedEmergencyCase) source;
                HospitalSystem.triggerAdminReassign(
                    sec.caseId, sec.patientName, sec.date, sec.time,
                    "Dr. " + newDoc.getName(), newDept);
            } else if (source instanceof HospitalSystem.SharedAppointment) {
                HospitalSystem.SharedAppointment sa = (HospitalSystem.SharedAppointment) source;
                HospitalSystem.triggerAdminReassign(
        ...
        btnPrioritize.addActionListener(e -> {
            int selectedIdx = emergencyTable.getSelectedRow();
            if (selectedIdx == -1 || selectedIdx >= rowSources.size()) return;
            Object source = rowSources.get(selectedIdx);
            ...
            String current = "";
            if (source instanceof EmergencyCase) {
                current = ((EmergencyCase) source).getSeverityLevel();
            } else if (source instanceof HospitalSystem.SharedEmergencyCase) {
                current = ((HospitalSystem.SharedEmergencyCase) source).severity;
            } else {
                JOptionPane.showMessageDialog(this, "Priority can only be changed for emergency cases.", "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            String sel = (String) JOptionPane.showInputDialog(this,
                "Change priority for this patient:", "Priority Rank",
                JOptionPane.QUESTION_MESSAGE, null, levels, current);
            if (sel == null || sel.equals(current)) return;

            if (source instanceof EmergencyCase) {
                ((EmergencyCase) source).setSeverityLevel(sel);
            } else if (source instanceof HospitalSystem.SharedEmergencyCase) {
                ((HospitalSystem.SharedEmergencyCase) source).severity = sel;
                HospitalSystem.notifyEmergencyListeners();
            }
            showActiveEmergencies();
        });
    ...
    public static void preseedRosterStore() {
        ...
        for (String dept : depts) {
            for (int i = 1; i <= 4; i++) {
                String id = "DOC-" + String.format("%02d", docIdx++);
    ...
    public static void main(String[] args) {
        ...
        for (String dept : depts) {
            for (int i = 1; i <= 4; i++) {
                String docId = "DOC-" + String.format("%02d", docIdx++);
                Doctor d = new Doctor(docId, docNames[nameIdx++], dept, "Room " + (100 + nameIdx));
        ...
        // Sync active status and shift ranges from DoctorRosterStore (persists across admin sessions)
        for (Doctor d : mockDoctors) {
            d.setActive(DoctorRosterStore.isDoctorActive(d.getId()));
            java.util.List<String> storedShifts = DoctorRosterStore.getShiftRanges(d.getId());
            if (!storedShifts.isEmpty()) {
                d.getShiftSlots().clear();
                for (String s : storedShifts) d.addShiftSlot(s);
            }
        }
