...
public class BookAppointmentPanel {
         ...
         if (selectedDay == -1) {
            ...
            JOptionPane.showMessageDialog(frame, "Please select a time slot.", "Error", 0);
             ...
             String dateStr = var10000 + "-" + String.format("%02d", cal.get(2) + 1) + "-" + String.format("%02d", selectedDay);
              if (HospitalSystem.isSlotBooked(var0[0], dateStr, selectedSlot)) {
                 JOptionPane.showMessageDialog(frame, "This slot has just been taken.\nPlease select a different time.", "Slot Unavailable", JOptionPane.WARNING_MESSAGE);
                 refreshSlotAvailability();
                 return;
              }
   ...
   static JPanel makeAvatar(String var0, final int var1) {
    ...
    static void refreshFromRosterStore() {
        ...
        if (!showingConfirm && mainContainer != null && listScroll != null && listScroll.getParent() == mainContainer) {
            JPanel p = new JPanel((LayoutManager)null);
            p.setBackground(BG);
            buildDoctorCards(p, Math.max(mainContainer.getWidth(), 600));
            listScroll.setViewportView(p);
            listScroll.revalidate();
            listScroll.repaint();
        }
    }
