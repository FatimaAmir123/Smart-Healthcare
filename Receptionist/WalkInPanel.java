...
public class WalkInPanel {
    ...
    public JPanel build() {
        ...
        btnAdmit.addActionListener(e -> {
            ...
            if (!doctor.equals("Unassigned") && HospitalSystem.isSlotBooked(ReceptionistDataStore.stripDr(doctor), ReceptionistDataStore.toYmd(date), time)) {
                ReceptionistUIHelper.showError(owner, "This time slot is already booked.\nPlease select a different slot.");
                return;
            }
            ...
            JOptionPane.showMessageDialog(owner,
                "email and sms have been sent to " + name, "Walk-in Admitted", JOptionPane.INFORMATION_MESSAGE);
            resetAll.run();
        });
