    ...
    static void showDialog(JFrame passedOwner, int row) {
        ...
        btnSave.addActionListener(e -> {
            ...
            if (!oldDoc.equals("Unassigned") && !oldDate.isEmpty() && !oldTime.isEmpty()
                ...
                // Remove old appointment from shared + doctor store WITHOUT triggering cancel flow
                HospitalSystem.unregisterAppointment(patientName, oldDateYmd, oldTime);
                Doctor.doctorDataStore rds = Doctor.doctorDataStore.get();
                javax.swing.table.DefaultTableModel ram = rds.getAppointmentModel();
                for (int ri = ram.getRowCount() - 1; ri >= 0; ri--) {
                    if (patientName.equals(ram.getValueAt(ri, 0)) && oldTime.equals(ram.getValueAt(ri, 1))) {
                        ram.removeRow(ri); break;
                    }
                }
                javax.swing.table.DefaultTableModel rhm = rds.getHistoryAppointmentModel();
                for (int ri = rhm.getRowCount() - 1; ri >= 0; ri--) {
                    if (patientName.equals(rhm.getValueAt(ri, 0)) && oldTime.equals(rhm.getValueAt(ri, 5))) {
                        rhm.removeRow(ri); break;
                    }
                }
                rds.notifyListeners();
            ...
            ReceptionistDataStore.apptModel.setValueAt(newPriority,  row, 6);
            // Push update back to the patient-side Appointment object
            HospitalSystem.syncReceptionistUpdateToPatient(
                patientName, apptId,
                ReceptionistDataStore.stripDr(newDoc), newDept,
                ReceptionistDataStore.toYmd(newDate), newTime);
