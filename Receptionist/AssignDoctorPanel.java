    ...
    public JPanel build(String prefilledPatient) {
        ...
        btnAssign.addActionListener(e -> {
            ...
            if (!docName.equals("Unassigned") && !date.isEmpty() && !time.isEmpty()) {
                String ymdCheck = ReceptionistDataStore.toYmd(date);
                String rawCheck = ReceptionistDataStore.stripDr(docName);
                // If updating an existing appointment, exclude its own slot
                boolean isSelfSlot = false;
                if (!apptRef.isEmpty()) {
                    for (int i = 0; i < ReceptionistDataStore.apptModel.getRowCount(); i++) {
                        if (ReceptionistDataStore.apptModel.getValueAt(i, 0).toString().equalsIgnoreCase(apptRef)) {
                            String oldDoc = ReceptionistDataStore.stripDr(ReceptionistDataStore.apptModel.getValueAt(i, 4).toString());
                            String oldDate = ReceptionistDataStore.toYmd(ReceptionistDataStore.apptModel.getValueAt(i, 2).toString());
                            String oldTime = ReceptionistDataStore.apptModel.getValueAt(i, 3).toString().trim();
                            if (rawCheck.equals(oldDoc) && ymdCheck.equals(oldDate) && time.equals(oldTime)) isSelfSlot = true;
                            break;
                        }
                    }
                }
                if (!isSelfSlot && HospitalSystem.isSlotBooked(rawCheck, ymdCheck, time)) {
                    JOptionPane.showMessageDialog(owner, "This time slot is already booked.\nPlease select a different slot.", "Slot Unavailable", JOptionPane.WARNING_MESSAGE);
                    return;
                }
            }
            ...
            JOptionPane.showMessageDialog(owner,
                "email and sms have been sent to " + pat, "Assignment Confirmed", JOptionPane.INFORMATION_MESSAGE);
