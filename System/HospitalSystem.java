...
public class HospitalSystem {
    ...
    /** Push receptionist's appointment edit back to the patient's Appointment object. */
    public static void syncReceptionistUpdateToPatient(String patientName, String apptId,
            String newDoctor, String newDept, String newDateYmd, String newTime) {
        try {
            for (Patient.Patient p : PatientGUI.allPatients) {
                if (p.getName().equals(patientName)) {
                    for (Appointment a : p.getAppointments()) {
                        if (a.getAppointmentId().equals(apptId)) {
                            a.setDoctorName(newDoctor);
                            a.setSpecialty(newDept);
                            a.setDate(newDateYmd);
                            a.setTimeSlot(newTime);
                            a.setStatus("Pending");
                            System.out.println("[Sync] Updated patient-side appointment " + apptId + " for " + patientName);
                            return;
                        }
                    }
                    break;
            ...
            System.out.println("[HospitalSystem] syncReceptionistUpdateToPatient: " + e.getMessage());
        }
    }
    ...
    public static void notifyEmergencyListeners() {
